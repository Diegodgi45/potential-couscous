...
/**
 * Best-effort heuristic to tell whether a TMDB result is anime: Japanese original language
 * AND tagged with the Animation genre. This intentionally excludes western cartoons (Animation
 * but not Japanese) and Japanese live-action (Japanese but not Animation).
 * @param {Array<number>} [genreIds]
 * @param {string} [originalLanguage]
 * @returns {boolean}
 */
function IsAnimeFromTMDB(genreIds, originalLanguage) {
  return (originalLanguage === "ja") && Array.isArray(genreIds) && genreIds.includes(TMDB_ANIMATION_GENRE_ID)
}
/**
 * Best-effort heuristic for Cinemeta results, which don't expose original language or numeric genre
 * ids: we only get a "genres" array of strings, so we just check for "Animation" in it.
 * @param {Array<string>} [genres]
 * @returns {boolean}
 */
function IsAnimeFromCinemeta(genres) {
  return Array.isArray(genres) && genres.includes("Animation")
}
...
class Metadata {
   ...
   * @param {boolean} adult 
   */
  constructor(imdbID, tmdbID, type, title, summary, releaseDate, adult, isAnime) {
    ...
    //true only when we could confirm (via TMDB genre+original language, or Cinemeta genre) that this is actually anime.
    //undefined/false means "treat as not anime" so we don't pollute results with unrelated movies/series.
    this.isAnime = isAnime === true;
  }
  shortPrint() {
 ...
 */
  static GetTMDBMetaFromTMDBID(tmdbID, mediaType, lang = undefined) {
    ...
    return new Promise((resolve, reject) => {
      fetch(reqURL, options).then((resp) => {
        ...
        const isAnime = IsAnimeFromTMDB(genreIds, data.original_language)
        if (mediaType === "movie") resolve(new Metadata(data.imdb_id, data.id, mediaType, data.title, data.overview, data.release_date, data.adult, isAnime))
        else {
          ...
          imdbIDPromise.then((imdbID) => {
            resolve(new Metadata(imdbID, data.id, mediaType, data.name, data.overview, data.first_air_date, data.adult, isAnime))
          })
        }
      }).catch(e => {
        reject(e)
  ...
  static ParseTMDBMeta(resultsArray, imdbID) {
    ...
    const isAnime = IsAnimeFromTMDB(first_item.genre_ids, first_item.original_language)
    return new Metadata(imdbID, first_item.id, first_item.media_type, title, first_item.overview, release_date, first_item.adult, isAnime)
  }
  /**
  ...
  static ParseCinemetaMeta(meta) {
    const isAnime = IsAnimeFromCinemeta(meta.genres)
    return new Metadata(meta.id, meta.moviedb_id, meta.type, meta.name, meta.description, meta.released || meta.releaseInfo, undefined, isAnime)
  }
  ...
  static GetSpecialMeta(imdbID, episode, type="series"){
    ...
    return fetch(reqURL).then((resp) => {
      ...
      const isAnime = IsAnimeFromCinemeta(data.meta.genres)
      return new Metadata(special?.id, data.meta.moviedb_id, type, special?.title || special?.name, special?.overview, special?.released, undefined, isAnime)
    })
  }
}
