...
const seasonResolver = require('../lib/seasonResolver.js')
...
function HandleStreamRequest(req, res, next) {
  ...
  if ((videoID?.startsWith("animeflv")) || (videoID?.startsWith("animeav1")) || (videoID?.startsWith("henaojara")) || (videoID?.startsWith("tioanime")) || (videoID?.startsWith("animejara")) || (videoID?.startsWith("jkanime"))) { //If we got an AnimeFLV or AnimeAV1 specific ID
    ...
    animeIMDBIDPromise.then((imdbID) => {
      ...
      if (!metadata.isAnime) {
        console.log(`\x1b[33m"${metadata.title}" doesn't look like anime (genre/language check failed), skipping anime site search\x1b[39m`)
        if (!res.headersSent) {
          res.header('Cache-Control', "max-age=86400, stale-while-revalidate=86400, stale-if-error=259200")
          res.json({ streams: [], message: "Not anime, skipped" })
          next()
        }
        return
      }
      ...
      //Translate season+episode (TMDB/Cinemeta numbering) into an absolute episode number (AnimeFLV/AnimeAV1/etc. numbering).
      //Returns undefined (and every site below falls back to the old title+season text search) if it can't be computed.
      const absoluteEpisodePromise = seasonResolver.GetAbsoluteEpisode(metadata.tmdbID, req.params.type, season, episode).catch(() => undefined)

      const animeFLVp = absoluteEpisodePromise.then((absoluteEpisode) => seasonResolver.ResolveEpisode({
        searchFn: (title) => animeFLVAPI.SearchAnimeFLV(title),
        getBySlugFn: (slug) => animeFLVAPI.GetAnimeBySlug(slug),
        fuzzysort, baseTitle, searchTermWithSeason: searchTerm, absoluteEpisode, rawEpisode: episode, type: req.params.type
      })).then(({ slug, episode: ep }) => {
        console.log('\x1b[36mGot AnimeFLV entry:\x1b[39m', slug, 'ep.', ep)
        return animeFLVAPI.GetItemStreams(slug, onlyInternal, ep)
      })
      const animeAV1p = absoluteEpisodePromise.then((absoluteEpisode) => seasonResolver.ResolveEpisode({
        searchFn: (title) => animeAV1API.SearchAnimeAV1(title, req.params.type),
        getBySlugFn: (slug) => animeAV1API.GetAnimeBySlug(slug),
        fuzzysort, baseTitle, searchTermWithSeason: searchTerm, absoluteEpisode, rawEpisode: episode, type: req.params.type
      })).then(({ slug, episode: ep }) => {
        console.log('\x1b[36mGot AnimeAV1 entry:\x1b[39m', slug, 'ep.', ep)
        return animeAV1API.GetItemStreams(slug, onlyInternal, ep)
      })
      const henaojarap = absoluteEpisodePromise.then((absoluteEpisode) => seasonResolver.ResolveEpisode({
        searchFn: (title) => henaojaraAPI.SearchHenaojara(title),
        getBySlugFn: (slug) => henaojaraAPI.GetAnimeBySlug(slug),
        fuzzysort, baseTitle, searchTermWithSeason: searchTerm, absoluteEpisode, rawEpisode: episode, type: req.params.type
      })).then(({ slug, episode: ep }) => {
        console.log('\x1b[36mGot Henaojara entry:\x1b[39m', slug, 'ep.', ep)
        return henaojaraAPI.GetItemStreams(slug, onlyInternal, ep)
      })
      const tioanimep = absoluteEpisodePromise.then((absoluteEpisode) => seasonResolver.ResolveEpisode({
        searchFn: (title) => tioanimeAPI.SearchTioAnime(title, req.params.type),
        getBySlugFn: (slug) => tioanimeAPI.GetAnimeBySlug(slug),
        fuzzysort, baseTitle, searchTermWithSeason: searchTerm, absoluteEpisode, rawEpisode: episode, type: req.params.type
      })).then(({ slug, episode: ep }) => {
      ...
      const jkanimep = absoluteEpisodePromise.then((absoluteEpisode) => seasonResolver.ResolveEpisode({
        ...
        console.log('\x1b[36mGot JKAnime entry:\x1b[39m', slug, 'ep.', ep)
        return jkanimeAPI.GetItemStreams(slug, onlyInternal, ep)
      })
      CombineStreams(animeFLVp, animeAV1p, henaojarap, tioanimep, animejarap, jkanimep).then((combinedStreams)=>{
        if (combinedStreams.length > 0) {
          console.log(`\x1b[36mGot ${combinedStreams.length} streams\x1b[39m`)
