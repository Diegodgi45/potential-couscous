...
async function ResolveEpisode({ searchFn, getBySlugFn, fuzzysort, baseTitle, searchTermWithSeason, absoluteEpisode, rawEpisode, type }) {
  ...
  //Fallback: old behavior - if fuzzysort finds nothing decent, still take a candidate (site catalogs/ordering
  //aren't always reliable), preferring one that matches the requested type (movie/series) when possible.
